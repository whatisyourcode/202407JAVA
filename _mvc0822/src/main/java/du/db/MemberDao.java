...
public class MemberDao {
	// SqlSessionFactory를 SqlMapConfig를 통하여 생성한다.
		...
		public List<MemberDto> selectList(int start, int listsize) {
			MemberMapper mm = session.getMapper(MemberMapper.class);
			return mm.selectList(start,start+listsize-1);
		}

		public MemberDto selectOne(int member_id) {
			MemberMapper mm = session.getMapper(MemberMapper.class);
			return mm.selectOne(member_id);
		}
		
		public void deleteMember(int member_id) {
			MemberMapper mm = session.getMapper(MemberMapper.class);
			mm.deleteMember(member_id);
		}
}
